...
export default function EditCountry() {
  ...
  // Отладочный лог
  useEffect(() => {
    console.log("EditCountry - nationId:", nationId);
    console.log("EditCountry - location.state:", location.state);
  }, [nationId, location.state]);
  ...
  useEffect(() => {
    if (location.state?.nationId) {
      ...
      console.log("Saved nationId to localStorage:", location.state.nationId);
    }
  }, [location.state]);
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    const url = `${apiUrl}/api/country/${id}?name=${encodeURIComponent(country.name)}&capital=${encodeURIComponent(country.capital)}&population=${encodeURIComponent(country.population)}&areaSquareKm=${encodeURIComponent(country.areaSquareKm)}&gdp=${encodeURIComponent(country.gdp)}`;
  ...
  const handleCancel = () => {
    console.log("Cancel clicked, navigating with nationId:", nationId);
